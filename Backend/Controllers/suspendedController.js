...
export const getAllSuspended = async (req, res) => {
  try {
    ...
    const clients = suspendedClients.map(client => client.get({ plain: true }));
...
export const unsuspend = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "The client is now unsuspended and status is set to 'Pending'." });
