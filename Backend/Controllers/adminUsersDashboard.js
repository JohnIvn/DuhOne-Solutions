    ...
    const users = usersAccount
      .map((account) => {
        const profile = userProfiles.find(
          (profile) => profile.userId === account.userId
        );
...
export const deleteAccount = async (req, res) => {
  ...
  try {
    ...
    if (!userAccount) {
      return res
        .status(404)
        .json({ message: "No account found with the provided userId" });
    }
    ...
    return res
      .status(200)
      .json({ message: `Successfully deleted userId: ${userIdToDelete}` });
  } catch (error) {
...
export const getUserProfileModal = async (req, res) => {
  ...
  try {
    ...
    if (!profile) {
      return res
        .status(400)
        .json({ message: "An error occured while geting the profile modal" });
    }
...
export const editUserProfileModal = async (req, res) => {
  const { id } = req.params;
  const { firstName, lastName, phoneNumber, street, city, barangay, zipCode } =
    req.body;
  ...
  try {
    ...
    if (result[0] === 0) {
      return res
        .status(404)
        .json({ message: "User profile not found or no changes made" });
    }
  ...
  try {
    if (!email && !userId) {
      return res
        .status(400)
        .json({
          message: "Please provide either an email or userId to search.",
        });
    }
